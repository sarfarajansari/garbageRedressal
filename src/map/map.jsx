...
const MapboxExample = () => {
  ...
  const { setActive: setActiveMenu } = useSidebar();

  function findGeographicalCenter() {
    const coordinates = complaints.map(
      (complaint) => complaint.address.coordinates
    );
  ...
  useEffect(() => {
    ...
    complaints.forEach((complaint) => {
      const marker = new mapboxgl.Marker({
        color: colors[complaint.currentStatus],
        rotation: 30,
      })
        .setLngLat(complaint.address.coordinates)
  ...
  return (
    <>
      ...
      <ComplaintDetails
        ...
        setComplaint={setActiveComplaint}
      />
    </>
  );
};
